...
public class EmojiVacation {
    ...
    private static void generateVacationPhoto(CanvasWindow canvas) {
        ...
        if (percentChance(50)) {
            addMountains(canvas, 100, randomDouble(100, 300), randomInt(5, 20));
        }

        addGround(canvas, 400);

        if (percentChance(60)) {
            addForest(canvas, 400, 50, randomInt(2,15));
        }
    ...
    private static void positionFamily(
        ...
        for (GraphicsGroup emojis: family) {
            double y = baselineY - emojis.getHeight();
            emojis.setPosition(currentX, y);
            currentX = currentX + emojis.getWidth() + spacing;
